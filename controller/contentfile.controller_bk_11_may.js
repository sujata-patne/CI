            ...
            if (req.session.UserName) {
                ...
                form.parse(req, function (err, fields, files) {
                    if (files.file) {
                        ...
                        var file_name = old_path.substr(index);
                        var file_namepath = files.file.name.substring(0, files.file.name.indexOf('.'));
                        var filenamedata = (fields.type != 'text')? (fields.cm_id + '_' + fields.type + '_' + Pad("0", fields.count, 2) + '.' + file_ext).toLowerCase() : (fields.cm_id + '_' + fields.ct_param_value + '_' + Pad("0", fields.count, 2) + '.' + file_ext).toLowerCase();
