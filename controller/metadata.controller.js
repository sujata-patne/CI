                        ...
                        LyricsLanguages: function (callback) {
                            if (status == "edit") {
                                var query = connection_ikon_cms.query('select * from (SELECT * FROM content_metadata where cm_id =? )meta ' +
                                    ...
                                    'inner join(select * from catalogue_detail )cd on(cd.cd_id = mlm.cmd_entity_detail) ' +
                                    'inner join(select * from catalogue_master where cm_name in ("Languages"))cm on(cm.cm_id =cd.cd_cm_id)', [req.body.Id], function (err, LyricsLanguages) {
                ...
                , function (err, cm_id) {
                    if (err) {
                        ...
                        var info = {
                            ...
                            action : 'addeditmetadata',
                            responseCode: 200,
                            message:  "Metadata for Contenttype " + contenttype1 + (req.body.state.indexOf("add") > -1 ? " added" : " updated") + " successfully"
                        }
                        wlogger.info(info); // for information
