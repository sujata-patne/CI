            ...
            if (req.session.UserName) {
                ...
                form.parse(req, function (err, fields, files) {
                    if (files.file) {
                        ...
                        var file_namepath = files.file.name.substring(0, files.file.name.indexOf('.')).replace(/ /g,"_");
                        var append_filename = fields.cm_title.replace(/ /g,"_");

                        var filenamedata = (fields.cm_id + '_thumb_' + fields.width + "_" + fields.height + "_" + append_filename + '.' + file_ext).toLowerCase();
            ...
            if (req.session.UserName) {
                ...
                form.parse(req, function (err, fields, files) {
                    if (files.file) {
                        ...
                        var filenamedata = (fields.cm_id + '_' + fields.width + "_" + fields.height + "_" + append_filename + '.' + file_ext).toLowerCase();
                                                                ...
                                                                var query = connection_ikon_cms.query('SELECT * FROM content_files_thumbnail WHERE cft_cm_id= ? ', [fields.cm_id], function (err, filedata) {
                                                                    if (err) {
                                                                                                        ...
                                                                                                        if (thumblength == tf) {
                                                                                                            var query = connection_ikon_cms.query('UPDATE content_metadata SET cm_state=? ,cm_modified_on = ? , cm_modified_by = ? WHERE cm_id=?', [cm_state, new Date(), req.session.UserName, fields.cm_id], function (err, Templates) {
                                                                                                                if (err) {
                                                                                                                    ...
                                                                                                                    var query = connection_ikon_cms.query('select * from (SELECT cm_id FROM content_metadata where cm_id =? )meta ' +
                                                                                                                        'inner join(select * from content_files where cf_original_processed = 1)files on(files.cf_cm_id = meta.cm_id) ' +
                                                                                                                        'inner join(select ct_group_id ,group_concat(ct_param) as ct_param,group_concat(ct_param_value) as ct_param_value ' +
                                                                                                                        'from content_template group by ct_group_id)template on(template.ct_group_id =files.cf_template_id)', [fields.cm_id], function (err, Files) {
                                                                                                                        if (err) {
                                                                                                                            connection_ikon_cms.release();
                                                                                                                            res.status(500).json(err.message);
                                                                                                                        }
                                                                        ...
                                                                        function formatloop(index) {
                                                                            ...
                                                                            if (match) {
                                                                                var subfile = (fields.cm_id + '_' + width + "_" + height + "_" + append_filename + '.' + file_ext).toLowerCase();
                                                                                ...
                                                                                if (!(width == fields.width && height == fields.height)) {
                                                                                    if (1280 == fields.width && 1280 == fields.height) {
                                                                                        var thumbname = (fields.cm_id + '_thumb_' + width + "_" + height + "_" + append_filename + '.' + file_ext).toLowerCase();
                                                            ...
                                                            var query = connection_ikon_cms.query('SELECT * FROM content_files_thumbnail WHERE cft_cm_id= ? ', [fields.cm_id], function (err, filedata) {
                                                                if (err) {
                                                                    ...
                                                                    if (IsThumb100) {
                                                                        ...
                                                                        var subfile = (fields.cm_id + '_thumb_' + 100 + "_" + 100  + "_" + append_filename + '.' + 'jpg').toLowerCase();
                                                                    ...
                                                                    if (IsThumb125) {
                                                                        var subfile = (fields.cm_id + '_thumb_' + 125 + "_" + 125 + "_" + append_filename + '.' + 'jpg').toLowerCase();
                                                                    ...
                                                                    if (IsThumb150) {
                                                                        var subfile = (fields.cm_id + '_thumb_' + 150 + "_" + 150 + "_" + append_filename + '.' + 'jpg').toLowerCase();
            ...
            if (req.session.UserName) {
                ...
                form.parse(req, function (err, fields, files) {
                    if (files.file) {
                        ...
                        var filenamedata = (fields.cm_id + '_' + fields.other + '_' + append_filename + '.' + file_ext).toLowerCase();
            ...
            if (req.session.UserName) {
                ...
                form.parse(req, function (err, fields, files) {
                    if (files.file) {
                        ...
                        var index = old_path.lastIndexOf('/') + 1;
                        var file_name = old_path.substr(index);
                        var file_namepath = files.file.name.substring(0, files.file.name.lastIndexOf('.')).replace(/ /g,"_");
                        var append_filename = fields.cm_title.replace(/ /g,"_");

                        var filenamedata = (fields.cm_id + '_' + 'app' + '_' + Pad("0", fields.count, 2) + '_' + append_filename + '.' + file_ext).toLowerCase();
            ...
            if (req.session.UserName) {
                ...
                form.parse(req, function (err, fields, files) {
                    if (files.file) {
                        ...
                        var file_namepath = files.file.name.substring(0, files.file.name.indexOf('.')).replace(/ /g,"_");
                        var append_filename = fields.cm_title.replace(/ /g,"_");
                        var filenamedata = (fields.cm_id + '_' + fields.ct_param_value + '_' + Pad("0", fields.count, 2) + '_' + append_filename + '.' + file_ext).toLowerCase();
            ...
            if (req.session.UserName) {
                ...
                form.parse(req, function (err, fields, files) {
                    if (files.file) {
                        ...
                        var filenamedata = (fields.cm_id + '_' + fields.ct_param_value + '_' + ticks + '_' + append_filename + '.' + file_ext).toLowerCase();
            ...
            if (req.session.UserName) {
                ...
                form.parse(req, function (err, fields, files) {
                    if (files.file) {
                        ...
                        var filenamedata = (fields.cm_id + '_' + fields.type + '_' + Pad("0", fields.count, 2) + '_' + append_filename + '.' + file_ext).toLowerCase();
