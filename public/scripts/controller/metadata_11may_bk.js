...
myApp.controller('metadataCtrl', function ($scope, $state, $http,$window, $stateParams, ngProgress,  Metadatas, _, Icon) {
    ...
    $scope.submitForm = function (isValid) {
        if (isValid) {
            ...
            var flag = (ReleaseYear > 1949 && ReleaseYear < (year + 2)) ? Datewithouttime($scope.Startdate) <= Datewithouttime($scope.Expirydate) ? Datewithouttime($scope.PropertyStartDate) <= Datewithouttime($scope.Startdate) && Datewithouttime($scope.PropertyEndDate) >= Datewithouttime($scope.Expirydate) ? $scope.RightSettingShow ? $scope.SelectedCountryRights.length > 0 ? $scope.SelectedChannelRights.length > 0 ? $scope.Duration ? DurationCheck($scope.Duration) : "" : "Please Select Channel Distribution rights." : "Please Select Country Distribution rights." : "" : "Start & Expiry date should be within limit of Property limits." : "Expire date must be equal or greater than start date." : "Release Year must be between 1950 to current year + 1.";
            if (flag == "") {
                var Rights = [];
                ...
                if ($scope.CurrentPage.indexOf("edit") > -1 && $scope.UserRole == "Moderator" && addedrights.length > 0) {
                    ...
                    try {
                        var meta = {
                            ...
                            cm_display_title: $scope.DisplayTitle,
                            cm_mood: $scope.SelectedMood,
                            cm_release_date: getDate($scope.ReleaseYear),
