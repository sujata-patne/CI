...
myApp.controller('content-filesCtrl', function ($scope, $state, $http, $stateParams, ngProgress, $window, ContentFile, _, Icon, Upload,Excel,$q) {
    ...
    $scope.videofileuploader = function (files) {
        ...
        if ($scope.videofile) {
            if (getExtension($scope.videofile.name).toLowerCase() == "mp4") {
                ...
                if (match) {
                    $scope.VideoFiles.push({ fileCategory:1, file: $scope.videofile, type: 'Video', ct_group_id: match.ct_group_id, cm_id: $scope.MetaId, width: 640, height: 320, other: null })
                }
            }
            else {
    ...
    $scope.audiofileuploader = function (files) {
        ...
        if ($scope.audiofile) {
            if (getExtension($scope.audiofile.name).toLowerCase() == "mp3") {
                ...
                if (match) {
                    $scope.AudioFiles.push({fileCategory:2, file: $scope.audiofile, type: 'Audio', ct_group_id: match.ct_group_id, cm_id: $scope.MetaId, width: null, height: null, other: 128 })
                }
            }
            else {
    ...
    $scope.audiozipfileuploader = function (audiofile, cm_id, ct_group_id, cd_name, MetaId, ct_param, ct_param_value) {
        if (audiofile) {
            if (getExtension(audiofile.name).toLowerCase() == "zip") {
                $scope.AudioZipFiles.push({ fileCategory:2, file: audiofile, type: 'audiozip', ct_param: ct_param, ct_param_value: ct_param_value, ct_group_id: ct_group_id, cm_id: cm_id, width: null, height: null, other: cd_name, langaugemetaid: MetaId })
            }
            else {
                    ...
                    if (supporting_image_limit - otherimages.length - $scope.gameimagefile.length >= 0) {
                        ...
                        if (match) {
                            ...
                            function gameimageloop(gi) {
                                $scope.GameImageFiles.push({fileCategory:2, count: (otherimages.length + $scope.GameImageFiles.length + 1), file: $scope.gameimagefile[gi], type: 'GameImage', ct_group_id: match.ct_group_id, cm_id: $scope.MetaId, width: null, height: null, other: 128 })
                                gi = gi + 1;
                    ...
                    if ((video_download_limit - othervideos.length - $scope.gamevideofile.length) >= 0) {
                        ...
                        if (match) {
                            ...
                            function gamevideoloop(gv) {
                                $scope.GameVideoFiles.push({ fileCategory:2, count: (othervideos.length + $scope.GameVideoFiles.length + 1), file: $scope.gamevideofile[gv], type: 'gamevideo', ct_group_id: match.ct_group_id, cm_id: $scope.MetaId, width: null, height: null, other: null })
                                gv = gv + 1;
            ...
            if ((text_limit - texts.length - textfile.length) >= 0) {
                _.each(textfile, function (val) {
                    ...
                    if (getExtension(val.name).toLowerCase() == "txt") {
                        $scope.TextFiles.push({ fileCategory:1, count: (texts.length + count.length + 1), file: val, type: 'text', ct_param: ct_param, ct_param_value: ct_param_value, ct_group_id: ct_group_id, cm_id: cm_id, width: null, height: null, other: cd_name, langaugemetaid: MetaId })
                    }
                    else {
        ...
        if (langsupportfile) {
            ...
            if ((text_limit - langs.length - langsupportfile.length) >= 0) {
                _.each(langsupportfile, function (val) {
                    ...
                    if (getExtension(val.name).toLowerCase() == "txt") {
                        $scope.LangSupportFiles.push({fileCategory:2, count: (langs.length + count.length + 1), file: val, type: 'text', ct_param: ct_param, ct_param_value: ct_param_value, ct_group_id: ct_group_id, cm_id: cm_id, width: null, height: null, other: cd_name, langaugemetaid: MetaId })
                    }
                    else {
    ...
    $scope.supportingfileuploader = function (files) {
        ...
        if ($scope.supportfile) {
            ...
            if (!$scope.commonfileerror) {
                var flag = true;
                var otherimages = _.where($scope.Files, { ct_param_value: 'otherimage' });
                var otheraudio = _.where($scope.Files, { ct_param_value: 'otheraudio' });
                var othervideos = _.where($scope.Files, { ct_param_value: 'othervideo' });
                ...
                if (imagecount != 0) {
                    if (!((supporting_image_limit - otherimages.length - imagecount) >= 0)) {
                        ...
                        if (otherimages.length >= supporting_image_limit) {
                            ...
                            if (otherimages.length == 0) {
                                $scope.commonfileerrormessage = "You can upload only " + (supporting_image_limit - otherimages.length) + " supporting images.";
                            }
                            else {
                                $scope.commonfileerrormessage = otherimages.length + " supporting image file already uploaded. You can upload only " + (supporting_image_limit - otherimages.length) + " supporting images.";
                            }
                        }
                        $scope.supportfile = '';
                        toastr.error($scope.commonfileerrormessage);
                    ...
                    if (!((audio_download_limit - otheraudio.length - audiocount) == 0)) {
                        ...
                        if (otheraudio.length >= audio_download_limit) {
                            $scope.commonfileerrormessage = otheraudio.length + " supporting audio file already uploaded. You can't upload anymore.";
                        }
                        else {
                            if (otheraudio.length == 0) {
                                $scope.commonfileerrormessage = "You can upload only " + (audio_download_limit - otheraudio.length) + " supporting audio.";
                            }
                            else {
                                $scope.commonfileerrormessage = otheraudio.length + " supporting audio file already uploaded. You can upload only " + (audio_download_limit - otheraudio.length) + " supporting audio.";
                            }
                        }
                        $scope.supportfile = '';
                        toastr.error($scope.commonfileerrormessage);
                    ...
                    if (!((video_download_limit - othervideos.length - videocount) == 0)) {
                        ...
                        if (othervideos.length >= video_download_limit) {
                            ...
                            if (othervideos.length == 0) {
                                $scope.commonfileerrormessage = "You can upload only " + (video_download_limit - othervideos.length) + " supporting video.";
                            }
                            else {
                                $scope.commonfileerrormessage = othervideos.length + " supporting video file already uploaded. You can upload only " + (video_download_limit - othervideos.length) + " supporting video.";
                            }
                        }
                        $scope.supportfile = '';
                        toastr.error($scope.commonfileerrormessage);
                ...
                if (flag) {
                    _.each($scope.supportfile, function (val) {
                        if (isImage(val.name)) {
                            ...
                            if (match) {
                                $scope.CommonFiles.push({fileCategory:2,  count: (otherimages.length + count.length + 1), file: val, type: 'image', ct_group_id: match.ct_group_id, cm_id: $scope.MetaId, width: null, height: null, other: 'common' })
                            }
                            ...
                            if (match) {
                                $scope.CommonFiles.push({fileCategory:2,  count: (otheraudio.length + count.length + 1), file: val, type: 'audio', ct_group_id: match.ct_group_id, cm_id: $scope.MetaId, width: null, height: null, other: 'common' })
                            }
                        }
                        else if (isVideo(val.name)) {
                            ...
                            if (match) {
                                $scope.CommonFiles.push({fileCategory:2,  count: (othervideos.length + count.length + 1), file: val, type: 'video', ct_group_id: match.ct_group_id, cm_id: $scope.MetaId, width: null, height: null, other: 'common' })
                            }
                        }
                    })
                }
                ...
                if (imagecount != 0) {
                    if (!((supporting_image_limit - otherimages.length - imagecount) >= 0)) {
                        ...
                        if (otherimages.length >= supporting_image_limit) {
                            ...
                            if (otherimages.length == 0) {
                                $scope.commonfileerrormessage = "You can upload only " + (supporting_image_limit - otherimages.length) + " common images.";
                            }
                            else {
                                if(supporting_image_limit > otherimages.length){
                                    $scope.commonfileerrormessage = otherimages.length + " common image file already uploaded. You can upload only " + Math.abs(supporting_image_limit - otherimages.length) + " common images.";
                                }else{
                                    $scope.commonfileerrormessage = otherimages.length + " common image file already uploaded. You can't upload anymore.";
                                }
                            }
                    ...
                    if (!((video_download_limit - othervideos.length - videocount) >= 0)) {
                        ...
                        if (othervideos.length >= video_download_limit) {
                            ...
                            if (othervideos.length == 0) {
                                $scope.commonfileerrormessage = "You can upload only " + (video_download_limit - othervideos.length) + " common video.";
                            }
                            else {
                                if(video_download_limit > othervideos.length){
                                    $scope.commonfileerrormessage = othervideos.length + " common video file already uploaded. you can upload only " + Math.abs(video_download_limit - othervideos.length) + " common video.";
                                }else{
                                    $scope.commonfileerrormessage = othervideos.length + " common video file already uploaded. You can't upload anymore.";
                                }
                            }
        ...
        if ($scope.TypeName == "Imagery") {
            ...
            if ($scope.wallpaperfile  || $scope.CommonFiles.length > 0 || $scope.thumbfile) {
                if ($scope.thumberror || $scope.wallpapererror) {
                    ...
                    function WallpaperUpload(wu) {
                        ...
                        ContentFile.Upload('/upload' + $scope.TypeName, { fileCategory:data.fileCategory,file: data.file, cm_title: $scope.cm_title, other: data.other, TypeName: $scope.TypeName, MetaDataId: $scope.MetadataId, cm_id: $scope.MetaId, width: data.width, height: data.height, ct_group_id: data.ct_group_id }, function (resp) {
                    ...
                    function WallpaperCommonUpload(tcu) {
                    ...
                    ThumbUpload(0, {upload:'thumb'}, function (data) {
                        WallpaperUpload(0, {upload:'main'}, function (data) {
                            SupportingCommonUpload(0, {upload:'common'}, function (data) {
            ...
            if ($scope.videofile || $scope.CommonFiles.length > 0 || $scope.thumbfile) {
                if ($scope.thumberror || $scope.videoerror) {
                    ...
                    function VideoCommonUpload(tcu) {
                        var data = $scope.CommonFiles[tcu];
                        ContentFile.Upload('/uploadotherfiles', {
                            fileCategory:data.fileCategory,
                            count: data.count,
            ...
            //if ($scope.AudioZipError.length == 0) {
             if ($scope.audiofile || $scope.CommonFiles.length > 0 || $scope.thumbfile || $scope.LangSupportFiles.length > 0 || ($scope.AudioZipFiles.length > 0)) {
            ...
            if ($scope.thumbfile || $scope.gameimagefile || $scope.gamevideofile || $scope.gameappfile) {
                if ($scope.thumberror || $scope.gameimageerror || $scope.gamevideoerror || $scope.gameapperror) {
                    ...
                    function GameImageUpload(gi) {
                        ...
                        ContentFile.Upload('/uploadotherfiles', { fileCategory:data.fileCategory, count: data.count, file: data.file, cm_title: $scope.cm_title, other: data.other, type: 'image', TypeName: $scope.TypeName, MetaDataId: $scope.MetadataId, cm_id: $scope.MetaId, width: data.width, height: data.height, ct_group_id: data.ct_group_id }, function (resp) {
                            $scope.Files = resp.data.Files;
                    ...
                    function GameVideoUpload(gv) {
                        ...
                        ContentFile.Upload('/uploadotherfiles', { fileCategory:data.fileCategory, count: data.count, file: data.file, cm_title: $scope.cm_title, other: data.other, type: 'video', TypeName: $scope.TypeName, MetaDataId: $scope.MetadataId, cm_id: $scope.MetaId, width: data.width, height: data.height, ct_group_id: data.ct_group_id }, function (resp) {
                            $scope.Files = resp.data.Files;
                    ...
                    function GameAppUpload(ga) {
                        ...
                        ContentFile.Upload('/upload' + $scope.TypeName, { fileCategory:data.fileCategory, count: data.count, Handsets: handsets.toString(), file: data.file, cm_title: $scope.cm_title, other: data.other, type: 'app', TypeName: $scope.TypeName, MetaDataId: $scope.MetadataId, cm_id: $scope.MetaId, width: data.width, height: data.height, ct_group_id: data.ct_group_id }, function (resp) {
                            $scope.Files = resp.data.Files;
                ...
                if ($scope.thumbfile ||  ($scope.CommonFiles.length >0)  || ($scope.TextFiles.length > 0 || $scope.commonfileerror)) {
                    if ($scope.thumberror || $scope.commonfileerror) {
                        ...
                        function TextCommonUpload(tcu) {
                            ...
                            var data = $scope.CommonFiles[tcu];
                            ContentFile.Upload('/uploadotherfiles', { fileCategory:data.fileCategory,count: data.count, file: data.file, cm_title: $scope.cm_title, other: data.other, type: data.type, TypeName: $scope.TypeName, MetaDataId: $scope.MetadataId, cm_id: $scope.MetaId, width: data.width, height: data.height, ct_group_id: data.ct_group_id }, function (resp) {
                                $scope.Files = resp.data.Files;
    ...
    function SupportingCommonUpload(tcu,item,success) {
        if(item.upload === 'common' && $scope.CommonFiles && $scope.CommonFiles.length > 0) {
            var data = $scope.CommonFiles[tcu];
            ...
            ContentFile.Upload('/uploadotherfiles', {
                ...
                if (tcu == $scope.CommonFiles.length) {
                 ...
                 //   ngProgress.complete();
                    ...
                    SupportingCommonUpload(tcu, item, success);
                }
            }, function (error) {
                toastr.error(error);
                $scope.uploading = false;
    ...
    function SupportingTextFileUpload(tuf,item,success) {
        if(item.upload === 'text' && $scope.LangSupportFiles && $scope.LangSupportFiles.length > 0) {
            ...
            ContentFile.Upload('/uploadText', {
                ...
                $scope.Files = resp.data.Files;
                toastr.success(resp.config.data.file.name + ' language file uploaded successfully.');
                tuf = tuf + 1;
