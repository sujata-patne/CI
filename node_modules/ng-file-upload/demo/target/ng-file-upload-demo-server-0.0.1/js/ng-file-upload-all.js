...
ngFileUpload.service('UploadBase', ['$http', '$q', '$timeout', function ($http, $q, $timeout) {
  ...
  this.fromDataUrl = function (dataUrl) {
    return upload.dataUrltoBlob(dataUrl);
  };
...
ngFileUpload.service('Upload', ['$parse', '$timeout', '$compile', 'UploadResize', function ($parse, $timeout, $compile, UploadResize) {
  ...
  function generateDataUrls(files, attr, scope) {
    var shouldGenDataUrl = upload.attrGetter('ngfDataUrl', attr, scope);
    if (shouldGenDataUrl != null && shouldGenDataUrl !== false) {
      var noObjectUrl = upload.attrGetter('ngfNoObjectUrl', attr, scope);
      for (var i = 0; i < files.length; i++) {
        upload.dataUrl(files[i], noObjectUrl);
      }
    }
  }
  ...
  upload.updateModel = function (ngModel, attr, scope, fileChange, files, evt, noDelay) {
    ...
    if (upload.validate(newFiles, ngModel, attr, scope, function () {
        if (noDelay) {
          ...
          generateDataUrls(files, attr, scope);
...
(function () {
  ...
  ngFileUpload.filter('ngfFile', ['UploadDataUrl', function (UploadDataUrl) {
    return function (url) {
      if (angular.isString(url) && url.substring(0, 4) === 'data') {
        return UploadDataUrl.dataUrltoBlob(url);
      }
      return null;
    };
  }]);
